...
while True:
    ...
    if distance < threshold: detected = 1
    if distance > threshold: detected = 0
    
    if detected and last_state == 0: counter = counter + 1
    
   
    display.set_text(line=0, text='Distance: ' + str(distance), update=False)
    display.set_text(line=1, text='Counter: ' + str(counter), update=True)
